...
➜ rbd create --size 1024 kkk-test/lalala
root in summer in ~ via 🐍 v2.7.5 
➜ rbd -p kkk-test ls
lalala
root in summer in ~ via 🐍 v2.7.5 
...
rbd image 'lalala':
        ...
        flags: 
root in summer in ~ via 🐍 v2.7.5 
➜ rbd resize  --size 2048 kkk-test/lalala
...
### 移动块设备
- 原理其实就是快照的克隆
- 例子：我想把池`kkk-test`中的块`lalala`移动到池`kkk-PPP`中
...
```shell
root in summer in ~ via 🐍 v2.7.5 
➜ rbd clone kkk-test/lalala@lalalasnap kkk-PPP/newmewlalala
2021-05-11 15:43:21.973634 7f141be22900 -1 librbd: parent snapshot must be protected
rbd: clone error: (22) Invalid argument
root in summer in ~ via 🐍 v2.7.5 
❯ rbd snap protect kkk-test/lalala@lalalasnap
root in summer in ~ via 🐍 v2.7.5 
➜ rbd clone kkk-test/lalala@lalalasnap kkk-PPP/lalala
root in summer in ~ via 🐍 v2.7.5 
➜ rbd ls kkk-PPP
lalala
k38-307-101777431315714916604
...
➜ rbd showmapped
root in summer in ~ via 🐍 v2.7.5 
➜ rbd map --image kkk-test/lalala
/dev/cephrbd0
root in summer in ~ via 🐍 v2.7.5 
➜ rbd showmapped
id pool                             image  snap device        
0  kkk-test lalala -    /dev/cephrbd0 
```
...
❯ umount -l /rbdtest
root in summer in ~ via 🐍 v2.7.5 
❯ rbd unmap /dev/rbd/kkk-test/lalala
root in summer in ~ via 🐍 v2.7.5 
➜ rbd showmapped
root in summer in ~ via 🐍 v2.7.5 
➜ 
...
1. 重命名
```shell
rbd rename kkk-test/klve kkk-test/klve_-sysmag-del
```

2. 查看快照
```shell
rbd snap ls kkk-test/klve_-sysmag-del | /bin/grep  -v 'SIZE' | /usr/bin/awk   '{print $2}'
```

3. 查看block_name_prefix
```shell
rbd -p kkk-test info klve_-sysmag-del | /bin/grep  block_name_prefix | /usr/bin/awk  -F. '{print $2}'
```

4. 删除对象名称

```shell
rados -p kkk-test rm rbd_id.klve_-sysmag-del

rados -p kkk-test rm rbd_header.32fb66b8b4567
```

5. 从对象名称的对象图中删除键

```shell
rados -p kkk-test rmomapkey rbd_directory id_32fb66b8b4567

rados -p kkk-test rmomapkey rbd_directory name_klve_-sysmag-del
```
...
UUU
root in summer in ~ via 🐍 v2.7.5 
➜ rbd trash move kRTRT -p kkk-test
...
lalala
root in summer in ~ via 🐍 v2.7.5 
➜ rbd snap create --snap lalalasnap kkk-test/lalala
```
...
```shell
root in summer in ~ via 🐍 v2.7.5 
➜ rbd clone kkk-test/lalala@lalalasnap kkk-test/newlalala
root in summer in ~ via 🐍 v2.7.5 
➜ rbd ls kkk-test
lalala
newlalala
...
```shell
root in summer in ~ via 🐍 v2.7.5 
❯ rbd flatten kkk-test/newlalala
...
```shell
root in summer in ~ via 🐍 v2.7.5 
➜ rbd snap unprotect kkk-test/lalala@lalalasnap
```
...
```shell
root in summer in ~ via 🐍 v2.7.5 
➜ rbd snap rollback kkk-test/lalala@lalalasnap
...
```shell
root in summer in ~ via 🐍 v2.7.5 
➜ rbd snap rm kkk-test/lalala@lalalasnap
```
